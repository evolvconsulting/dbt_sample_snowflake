...
with customers as (
    ...
    ,x.full_address
    ,x.phone_number    
    --,x.account_balance -- add to fact_supplier
    ,hash(
        x.supplier_key
        ,n.nation_key
        ,r.region_key
...
{% if is_incremental() %}
  ...
  -- get all cusotmer records and row_hash to determine later if a merge is truly necessary
    , prior_vals as (
        ...
        from {{ ref("supplier") }} s
        left join {{this}} x on s.supplier_key = x.supplier_key
        where s.updated_by_timestamp >= (select max(updated_by_timestamp) from {{this}})
    )
...
{% if is_incremental() %}
  -- this filter will only be applied on an incremental run
    ...
    join prior_vals pv
        on x.supplier_key = pv.supplier_key
